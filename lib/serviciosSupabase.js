...
export async function registrarVentaConDetalles(datosVenta) {
  ...
  const numeroFactura = `FAC-MINI-${Date.now().toString().slice(-6)}`;
...
/**
 * Autenticar o verificar credenciales de usuario desde Supabase.
 */
export async function autenticarUsuarioPorCorreoYClave(correo, clave) {
  try {
    const { data, error } = await clienteSupabase
      .from('usuarios')
      .select('*')
    ...
    if (error) {
      console.log('Consulta de usuario en Supabase no disponible temporalmente, usando fallback local.');
      return null;
    }
